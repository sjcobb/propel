...
export class Transpiler {
  ...
  private static errorStackContainsMessage: boolean = (() => {
    const message = "this is a dummy message";
    try {
      throw new Error(message);
    } catch (e) {
      return e.stack.indexOf(message) !== -1;
    }
  })();
  ...
  formatError(error: any) {
    ...
    if (!Transpiler.errorStackContainsMessage) {
      let msg = "";
      if (error.constructor && error.constructor.name) {
        msg += error.constructor.name;
      } else {
        msg += "Error";
      }
      if (error.message !== undefined && String(error.message) !== "") {
        msg += `: ${error.message}`;
      }
      if (stack) {
        msg += "\n";
